...
public class ResDomainTreeAdapter<T> extends TreeListViewAdapter<T> {
    ...
    public ResDomainTreeAdapter(ListView mTree, Context context, List<T> datas,
        ...
        allNodeOKCallBack = (AllNodeOKCallBack) context;
    ...
    public View getConvertView(final Node node, final int position, View convertView, ViewGroup parent) {
        ...
        for (StationBean s : stationBeans) {
            if (s.getId().equals(node.getId())) {
                ...
                } else if ("DOMAIN".equals(s.getModel())) {
    ...
    public void checkAllChildren(Node node) {
        ...
        for (Node children : childrens) {
            if (children.isLeaf()) {
                for (StationBean s : ((List<StationBean>) originalDatas)) {
                    if (s.getId().equals(children.getId())) {
                        s.setCheck("true");
                    }
                }
            } else {
    ...
    public void cancelCheckAllChildren(Node node) {
        ...
        for (Node children : childrens) {
            if (children.isLeaf()) {
                ...
                cancelCheckAllChildren(children);
            }
        }
    }
    ...
    public void cancelParentCheck(Node node) {
        if (node.isRoot()) {
            ...
            if (parent.isRoot()) {
                for (StationBean s : ((List<StationBean>) originalDatas)) {
                    if (s.getId().equals(parent.getId())) {
                        s.setCheck("null");
                    }
                }
            } else {
