...
/**
 * Create Date: 2015-10-28<br>
 * Create Author: tWX318235<br>
 * Description :用于解决TextView中英混排换行问题
 */
public class StartCustomTextView extends TextView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int lines = getMaxLines() > contentList.size() ? contentList.size() : getMaxLines();
        for (int i = 0; i < lines; i++) {
            // 绘制一行
            float realDrawedWidth = 0 + leftPadding;
            LINE line = contentList.get(i);
            if (i == 0 && (getGravity() == Gravity.CENTER || getGravity() == Gravity.CENTER_VERTICAL)) {
                height += (getMaxLines() - lines) * (line.height + lineSpacing) / 2f;
            }
            for (int j = 0; j < line.line.size(); j++) {
                ob = line.line.get(j);
                width = line.widthList.get(j);
                if (ob instanceof String) {
                    String drawText = (String) ob;
                    if (contentList.size() > getMaxLines() && i + 1 == getMaxLines()) {
                        String point = "…";
                        String tempStr = drawText + point;
                        float finalWidth = paint.measureText(drawText);
                        float tempWidth = paint.measureText(tempStr);
                        while (tempWidth > finalWidth) {
                            drawText = drawText.substring(0, drawText.length() - 1);
                            tempStr = drawText + point;
                            tempWidth = paint.measureText(tempStr);
                        }
                        // String subStr = drawText.substring(0, drawText.length() - point.length());
                        canvas.drawText(tempStr, realDrawedWidth, height + line.height, paint);
                        break;
                    } else {
    ...
    private int measureContentHeight(int width) {
        ...
        for (int i = 0; i < obList.size(); i++) {
            ...
            if (width - drawedWidth < obWidth) {
                ...
                line = new LINE();
                // 控制绘制行数不超过实际显示行数
                isMeasureEnd = (contentList.size() >= getMaxLines());
            }
            drawedWidth += obWidth;
            ...
            if (isMeasureEnd) {// 如果判断出已经画完，就结束循环
                break;
            }
        }
    ...
    /**
     * 调用父类默认的方法设置文本内容和文本颜色，而不是用自定义的方法
     *
     * @param useDefault
     */
     ...
     * Description :存储ImageSpan及其开始结束位置
     */
    static class IS {
        public ImageSpan is;
        public int start;
