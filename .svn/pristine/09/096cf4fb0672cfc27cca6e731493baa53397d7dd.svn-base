    ...
    public static List<Node> filterVisibleNode(List<Node> nodes) {
        ...
        for (Node node : nodes) {
            ...
            if (node.isRoot() || node.isParentExpand()) {
                ...
                //判断是否是叶子节点且是电站
                if (node.getBean() instanceof PowerStationListBean.PowerStationBean){//如果是,隐藏该item(没办法,服务器返回了下面没有符合要求的电站的域名,只有自己过滤了)
                    PowerStationListBean.PowerStationBean powerStationBean= (PowerStationListBean.PowerStationBean) node.getBean();
                    if (!node.isLeaf() || "STATION".equals(powerStationBean.getModel())){
                        result.add(node);
                    }
                }else{
                    result.add(node);
