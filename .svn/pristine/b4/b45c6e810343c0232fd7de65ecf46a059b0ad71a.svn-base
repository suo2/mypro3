package com.huawei.solarsafe.utils.customview.treeview;
...
public class TreeHelper {
     ...
     */
    public static <T> List<Node> convertData2Node(List<T> datas)
    ...
    public static void setNodeIcon(Node node) {
        if (node.getChildren().size() > 0 && node.isExpand()) {
            node.setIcon(R.drawable.domain_zd_icon);
        } else if (node.getChildren().size() > 0 && !node.isExpand()) {
            node.setIcon(R.drawable.domain_zk_icon);
        } else
            node.setIcon(-1);
